...
(function () {
  ...
  const GENRES = ['Tous', 'Action', 'Horreur', 'Aventure', 'Famille'];
  const GCOLOR = { Action: '#234db4', Horreur: '#581383', Aventure: '#1b8a4b', Famille: '#c2410c' };
  ...
  const heart = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20s-7-4.4-7-9.3A3.7 3.7 0 0 1 12 8a3.7 3.7 0 0 1 7-2.3c0 4.9-7 9.3-7 9.3z"/></svg>';
  ...
  function apply() {
    ...
    gridEl.innerHTML = list.map((g, i) => `
      <a class="xcard reveal" data-d="${i % 4}" href="${g.href}">
        ...
        <button class="xcard__fav" aria-label="Favori" type="button">${heart}</button>
    ...
    gridEl.querySelectorAll('.xcard__fav').forEach(b => b.addEventListener('click', (e) => {
      e.preventDefault(); b.classList.toggle('on');
      b.style.background = b.classList.contains('on') ? 'var(--lime)' : '';
      b.style.color = b.classList.contains('on') ? 'var(--ink)' : '';
    }));
